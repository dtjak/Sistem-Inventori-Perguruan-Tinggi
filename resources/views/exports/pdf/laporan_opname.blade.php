...
<body>
    ...
    <div class="header">
        <h2>Laporan Rekapitulasi Riwayat Stock Opname</h2>
    ...
    <table class="items-table">
        <thead>
            <tr>
                ...
                <th style="width: 15%">Nomor Opname</th>
                <th style="width: 13%">Tanggal</th>
                <th style="width: 18%">Petugas Pelaksana</th>
                <th style="width: 37%">Catatan</th>
        ...
        <tbody>
            @foreach($data as $index => $item)
                <tr>
                    ...
                    <td class="font-monospace fw-bold">{{ $item->nomor_opname }}</td>
                    <td>{{ $item->tanggal->format('d/m/Y') }}</td>
                    <td>{{ $item->petugas->name ?? '-' }}</td>
                    <td>{{ $item->catatan ?: '-' }}</td>
                    <td class="text-center" style="text-transform: uppercase; font-weight: bold;">{{ $item->status }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
