@extends('layouts.app')
@section('title', 'Laporan Purchase Requisition')
@section('breadcrumb')
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h4 class="fw-bold mb-1">Laporan Purchase Requisition (PR)</h4>
        <p class="text-muted mb-0">Rekapitulasi pengajuan pengadaan pembelian barang inventori baru</p>
    </div>
    <div>
...
<div class="card data-card mb-4">
    <div class="card-header bg-white py-3">
        <form action="{{ route('laporan.pr') }}" method="GET" class="row g-2 align-items-center">
            <div class="col-md-3">
            ...
            <tbody>
                @forelse($data as $index => $item)
                    <tr>
                        ...
                        <td><span class="badge bg-light text-dark font-monospace">{{ $item->nomor_pr }}</span></td>
                        <td>{{ $item->tanggal->format('d M Y') }}</td>
                        <td>{{ $item->alasan }}</td>
                        <td>{{ $item->dibuatOleh->name ?? '-' }}</td>
                        <td class="fw-bold">Rp {{ number_format($item->total_estimasi, 0, ',', '.') }}</td>
                        <td>
                ...
                @empty
                    <tr>
                        <td colspan="7" class="text-center py-5 text-muted">
                            Tidak ada data Purchase Requisition.
                        </td>
                    </tr>
                @endforelse
            </tbody>
