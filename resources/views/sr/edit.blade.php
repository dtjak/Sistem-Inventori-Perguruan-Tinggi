...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item active">Edit SR</li>
@endsection
    ...
    <div class="col-xl-10">
        <div class="card data-card">
            <div class="card-header">
                <h5 class="mb-0 fw-bold"><i class="bi bi-pencil-square text-warning me-2"></i>Edit Store Requisition</h5>
            </div>
            <form action="{{ route('sr.update', $sr->id) }}" method="POST" id="srForm">
                @csrf
                @method('PUT')
                <div class="card-body">
                    <!-- Header SR -->
                    <div class="row g-3 mb-4">
                        <div class="col-md-4">
                            ...
                            <input type="date" name="tanggal" class="form-control @error('tanggal') is-invalid @enderror"
                                   value="{{ old('tanggal', $sr->tanggal->format('Y-m-d')) }}" required>
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="text" name="unit_peminjam" class="form-control @error('unit_peminjam') is-invalid @enderror"
                                   value="{{ old('unit_peminjam', $sr->unit_peminjam) }}" required>
                            @error('unit_peminjam')<div class="invalid-feedback">{{ $message }}</div>@enderror
                        </div>
                        <div class="col-md-4">
                            <label class="form-label fw-semibold">Pemohon</label>
                            <input type="text" class="form-control bg-light" value="{{ $sr->pemohon->name }}" readonly>
                        </div>
                        <div class="col-md-12">
                            <label class="form-label fw-semibold">Catatan</label>
                            <textarea name="catatan" class="form-control" rows="2"
                                      placeholder="Catatan tambahan...">{{ old('catatan', $sr->catatan) }}</textarea>
                        </div>
                    </div>
                    ...
                    <div id="detailContainer">
                        @foreach($sr->details as $index => $detail)
                            @php
                                $isAset = !empty($detail->aset_id);
                            @endphp
                            <div class="detail-row row g-2 align-items-center mb-2" data-index="{{ $index }}">
                                <div class="col-md-2">
                                    ...
                                    <select class="form-select form-select-sm type-select">
                                        <option value="barang" {{ !$isAset ? 'selected' : '' }}>Barang Habis Pakai</option>
                                        <option value="aset" {{ $isAset ? 'selected' : '' }}>Aset / Peralatan</option>
                                    </select>
                                </div>
                                <div class="col-md-4 item-barang-col {{ $isAset ? 'd-none' : '' }}">
                                    <label class="form-label small fw-semibold mb-1">Barang <span class="text-danger">*</span></label>
                                    <select name="details[{{ $index }}][barang_id]" class="form-select form-select-sm barang-select" {{ $isAset ? 'disabled' : 'required' }}>
                                        <option value="">-- Pilih Barang --</option>
                                        @foreach($barangs as $barang)
                                        <option value="{{ $barang->id }}" data-stok="{{ $barang->stok_saat_ini }}" data-satuan="{{ $barang->satuan }}" {{ $detail->barang_id == $barang->id ? 'selected' : '' }}>
                                            {{ $barang->kode_barang }} - {{ $barang->nama_barang }} (Stok: {{ $barang->stok_saat_ini }} {{ $barang->satuan }})
                                        </option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-md-4 item-aset-col {{ !$isAset ? 'd-none' : '' }}">
                                    <label class="form-label small fw-semibold mb-1">Aset <span class="text-danger">*</span></label>
                                    <select name="details[{{ $index }}][aset_id]" class="form-select form-select-sm aset-select" {{ !$isAset ? 'disabled' : 'required' }}>
                                        <option value="">-- Pilih Aset --</option>
                                        @foreach($asets as $aset)
                                        <option value="{{ $aset->id }}" data-lokasi="{{ $aset->lokasi }}" {{ $detail->aset_id == $aset->id ? 'selected' : '' }}>
                                ...
                                <div class="col-md-2">
                                    ...
                                    <input type="number" name="details[{{ $index }}][qty]" class="form-control form-control-sm" min="1" value="{{ $detail->qty }}" required>
                                </div>
                                <div class="col-md-3">
                                    <label class="form-label small fw-semibold mb-1">Keterangan</label>
                                    <input type="text" name="details[{{ $index }}][keterangan]" class="form-control form-control-sm" value="{{ $detail->keterangan }}" placeholder="Keterangan...">
                                </div>
